...
public class EthernetManager {
    ...
    public synchronized EthernetDevInfo getSavedConfig() {
        ...
        info.setRouteAddr(getSharedPreRoute());
    ...
    void configureInterface(EthernetDevInfo info) {
        try {
        	mNMService.disableIpv6(info.getIfName());
        } catch (RemoteException re){
            Log.e(TAG,"Disable IPV6 failed: " + re);
        }
        if (info.getConnectMode().equals(EthernetDevInfo.ETHERNET_CONN_MODE_DHCP)) {
            ...
            try {
                ...
                ifcg.setLinkAddress(new LinkAddress(NetworkUtils.numericToInetAddress(
                        info.getIpAddress()), NetworkUtils.netmaskIntToPrefixLength(numericIPToInt(info.getNetMask()))));
                //NetworkUtils.netmaskIntToPrefixLength(NetworkUtils.inetAddressToInt(NetworkUtils.numericToInetAddress(info.getNetMask())));
                ifcg.setInterfaceUp();
                mNMService.setInterfaceConfig(info.getIfName(), ifcg);

                if (info.getRouteAddr() != null) {
                	mNMService.addRoute(info.getIfName(), 
    ...
    public EthernetDevInfo getDhcpInfo() {
        ...
        try {
        	...
        	for (int i = 0; i < routeinfos.length; i++) {
        	    Log.d(TAG,"Route info: " + routeinfos[i]);
        		int route = NetworkUtils.inetAddressToInt(routeinfos[i].getGateway());
        		//Log.d(TAG, "check route : " + ((route >> 24) & 0xff) + "." + ((route >> 16) & 0xff)+ "." + ((route >> 8) & 0xff) + "." + (route & 0xff));
        		Log.d(TAG, "check route : " + (route & 0xff) + "." + ((route >> 8) & 0xff) + "." + ((route >> 16) & 0xff)+ "." + ((route >> 24) & 0xff));
        		String routeString = (route & 0xff) + "." + ((route >> 8) & 0xff) + "." + ((route >> 16) & 0xff)+ "." + ((route >> 24) & 0xff);
        		if (!routeString.equals("0.0.0.0")) {
        			infotemp.setRouteAddr(routeString);
        			break;
				}
        	}
        } catch (Exception re){
    ...
    public synchronized void updateDevInfo(EthernetDevInfo info) {
        ...
        if (info.getRouteAddr() != null) {
        	SystemProperties.set("net." + info.getIfName() + ".route", info.getRouteAddr());
		}
    }
    ...
    public void sharedPreferencesStore(EthernetDevInfo info){
        ...
        try {
            ...
            editor.putString("mRoute",info.getRouteAddr());
    ...
    public String getSharedPreRoute(){
        String temp = null;
        try {
            temp = sharedPreferences().getString("mRoute",null);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return temp;
